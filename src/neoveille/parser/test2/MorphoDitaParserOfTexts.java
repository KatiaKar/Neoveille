...
public class MorphoDitaParserOfTexts extends ParserOfTexts{
	...
	void runTagger(String inputFile, String outputVerticalizedDir, String outputXMLverticalizedDir, String morphoDitaTagger,String language) throws IOException, InterruptedException {
		...
		for(int i=0; i<listOfFiles.size(); i++){
			...
			current_file = listOfFiles.get(i).substring(listOfFiles.get(i).lastIndexOf("/"),listOfFiles.get(i).length());
		...
		tmp_current_directory = outputXMLverticalizedDir + "/Concatenation/" + language + "/tmp_all.txt";
		current_directory = outputXMLverticalizedDir + "/Concatenation/" + language + "/all.txt";
